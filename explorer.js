...
function path2short(path, escape = false) {
  ...
  const soft = `${
    prefix2parentfolder(fullpath2pathname(path)) + pathHellip
  }/${fullpath2filename(path)}`;
  if (soft.length < pathLimit && soft.length > 2)
    return escape ? htmlEscape(soft) : soft;

  const hard = `${
    path.substring(0, path.indexOf("/") + 1) + pathHellip
  }/${fullpath2filename(path)}`;
  const rc =
    hard.length < pathLimit ? hard : path.substring(0, pathLimit) + pathHellip;
  return escape ? htmlEscape(rc) : rc;
}
...
function SharedService($rootScope) {
  ...
  shared.downloadObjects = (bucket, keys) =>
    $rootScope.$broadcast("broadcastDownloadObjects", { bucket, keys });
...
function ViewController($scope, SharedService) {
  ...
  $scope.renderFolder = (data, _type, full) =>
    full.CommonPrefix ? "" : fullpath2pathname(data, true);
  ...
  $scope.download = () => {
    DEBUG.log("Download:", $scope.view.keys_selected);
    if ($scope.view.keys_selected.length > 0) {
      SharedService.downloadObjects(
        $scope.view.settings.bucket,
        $scope.view.keys_selected
      );
    }
  ...
  $tb.DataTable({
    ...
    aoColumnDefs: [
      ...
      },
      {
        aTargets: [5],
        mData: "StorageClass",
        mRender: this.renderStorageClass,
      },
      {
        aTargets: [6],
        mData: this.dataSize,
...
function UploadController($scope, SharedService) {
  ...
  $scope.uploadFiles = (Bucket, prefix) => {
    ...
    $scope.upload.files.forEach((file, ii) => {
      ...
      const params = {
        ...
        Key:
          (prefix || "") +
          (file.file.fullPath ? file.file.fullPath : file.file.name),
      ...
      const funcsend = (err, data) => {
        ...
        if (err) {
          ...
          $scope.$apply(() => {
            ...
            $scope.upload.uploads = $scope.upload.uploads.filter(
              (f) => f !== upl
            );
          });
        }
  ...
  async function getAllFileEntries(dataTransferItemList) {
    ...
    for (let i = 0; i < dataTransferItemList.length; i++) {
      ...
      queue.push(
        typeof dtItem.webkitGetAsEntry === "function"
          ? dtItem.webkitGetAsEntry()
          : dtItem.getAsEntry()
      );
    }
...
function TrashController($scope, SharedService) {
  ...
  $scope.$on("broadcastTrashObjects", (e, args) => {
    ...
    for (let ii = 0; ii < args.keys.length; ii++) {
      ...
      const object = path2short(
        isfolder(obj.Key) ? prefix2folder(obj.Key) : fullpath2filename(obj.Key)
      );

      const folder = path2short(
        isfolder(obj.Key)
      ...
      const objectclass = isfolder(obj.Key) ? "" : mapStorage[obj.StorageClass];

      const size = isfolder(obj.Key) ? "" : bytesToSize(obj.Size);
  ...
  $scope.$on("broadcastDownloadObjects", (e, args) => {
    DEBUG.log("TrashController", "broadcast download objects", args);

    $scope.download.objects = [];

    // Populate scope download object array with objects to be deleted
    for (let ii = 0; ii < args.keys.length; ii++) {
      const obj = args.keys[ii];
      DEBUG.log("Object to be downloaded:", obj);
